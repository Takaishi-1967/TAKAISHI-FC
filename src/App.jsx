...
function App() {
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem("takaishi_admin") === "true") {
      setIsAdmin(true);
    }
  }, []);
  ...
  async function movePlayer(playerId, direction) {
    ...
    // 新しい順番を振り直す
    const reorderedPlayers = updatedPlayers.map(
      (player, index) => ({
        ...player,
        sort_order: index + 1,
      })
    ...
    // 裏でSupabaseに保存
    const updates = reorderedPlayers.map((player) =>
      supabase
        .from("players")
        .update({
    ...
    if (error) {
      ...
      alert(
        "並び替えの保存に失敗しました。再読み込みすると元に戻る可能性があります。"
      );
  ...
  function getAttendance(playerId, eventId) {
